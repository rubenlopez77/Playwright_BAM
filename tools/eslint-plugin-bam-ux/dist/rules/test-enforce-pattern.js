...
const isGetPageCall = (node) => node?.type === "CallExpression" &&
    node.callee?.type === "MemberExpression" &&
    node.callee.object?.type === "ThisExpression" &&
    isIdentifier(node.callee.property, "getPage");
const isExpectCall = (node) => node?.type === "CallExpression" &&
...
const rule = {
    meta: {
        type: "problem",
        docs: {
            description: "BAM declarative test pattern enforcement (modo exhaustivo).",
            recommended: false,
        },
        messages: {
            firstStmtPattern: "El primer statement debe ser const <var> = this.getPage(Clase);",
            onlyUserCalls: "Solo se permiten user.metodo() o expect() después de obtener la página.",
        },
        schema: [],
    },
    create(context) {
